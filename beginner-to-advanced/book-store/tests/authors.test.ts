...
describe("GET /authors", () => {
  it("is public and paginates", async () => {
    await seedAuthor();
    await seedAuthor();
    const res = await api.get("/authors?limit=1");
  ...
  it("returns 404 for an unknown author", async () => {
    const res = await api.get("/authors/00000000-0000-0000-0000-000000000000");
...
describe("GET /authors/:id/books", () => {
  it("returns the author's books, paginated", async () => {
    ...
    const res = await api.get(`/authors/${author.id}/books`);
    expect(res.status).toBe(200);
    expect(res.body.pagination.total).toBe(2);
  });
});
...
describe("POST /authors", () => {
  it("requires authentication", async () => {
    const res = await api.post("/authors").send({ name: "A", email: "a@t.com" });
...
describe("author ownership + soft delete", () => {
  it("owner deletes; another publisher is forbidden; delete hides it", async () => {
    ...
    const gone = await api.get(`/authors/${author.id}`);
